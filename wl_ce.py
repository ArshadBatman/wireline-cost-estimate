...
reference_wells = {
    "Reference Well A": {
        "Hole Sections": {
            "12.25\"": ["PEX-AIT (150DegC Maximum)", "DSI-Dual OBMI (150DegC Maximum)",
                        "MDT Pretest and Sampling (MDT-LFA-QS-XLD-MIFA-2MS)", "XL Rock (150 DegC Maximum)"],
            "8.5\"": ["PEX-AIT (150DegC Maximum)", "DSI-Dual OBMI (150DegC Maximum)",
                      "MDT Pretest and Sampling (MDT-LFA-QS-XLD-MIFA-2MS)", "XL Rock (150 DegC Maximum)"]
        },
        "Additional Items": ["Pipe Conveyed Logging", "FPIT & Back-off services / Drilling contingent Support Services",
                             "Unit, Cables & Conveyance", "Personnel"],
        "Quantity of Tools": 2,
        "Total Months": 1,
...
if uploaded_file:
    ...
    selected_ref_well = st.sidebar.selectbox("Select Reference Well", ref_well_options)

    # Hole sections setup
    st.sidebar.header("Hole Sections Setup")
    if selected_ref_well != "None":
        ref_holes = reference_wells[selected_ref_well]["Hole Sections"]
        hole_sizes = list(ref_holes.keys())
    else:
    ...
    for tab, hole_size in zip(tabs, hole_sizes):
        with tab:
            ...
            total_depth = st.sidebar.number_input(f"Total Depth (ft) ({hole_size})", min_value=0,
                                                  value=reference_wells[selected_ref_well]["Depths"].get(hole_size, 5500) if selected_ref_well != "None" else 5500, key=f"depth_{hole_size}")
            ...
            special_cases_map = {
                "STANDARD WELLS": {
                    "PEX-AIT (150DegC Maximum)": ["AU14: AUX_SURELOC","GR1: GR_TOTL","NE1: NEUT_THER","DE1: DENS_FULL","RE1: RES_INDU"],
                    "DSI-Dual OBMI (150DegC Maximum)": ["AU14: AUX_SURELOC","GR1: GR_TOTL","AU3: AUX_INCL","AC3: ACOU_3","AU2: AUX_PCAL","PP7: PROC_PETR7",
                                                       "PA7: PROC_ACOU6","PA11: PROC_ACOU13","PA12: PROC_ACOU14","IM3: IMAG_SOBM","PI1: PROC_IMAG1","PI2: PROC_IMAG2",
                                                       "PI7: PROC_IMAG7","PI8: PROC_IMAG8","PI9: PROC_IMAG9","PI12: PROC_IMAG12","PI13: PROC_IMAG13"],
                    "MDT Pretest and Sampling (MDT-LFA-QS-XLD-MIFA-2MS)": ["AU14: AUX_SURELOC","FP25: FPS_SCAR","FP18: FPS_SAMP","FP19: FPS_SPHA","FP23: FPS_TRA",
            ...
            # Preselect tools from Reference Well
            preselected_tools = []
            if selected_ref_well != "None" and hole_size in reference_wells[selected_ref_well]["Hole Sections"]:
                preselected_tools = reference_wells[selected_ref_well]["Hole Sections"][hole_size]

            # Filter to avoid Streamlit multiselect error
            preselected_tools_filtered = [t for t in preselected_tools if t in code_list_with_special]

            selected_codes = st.multiselect(
                "Select Tools (by Specification 1)",
                code_list_with_special,
                default=preselected_tools_filtered,
            ...
            if not df_tools.empty:
                ...
                # Operating and Rental Charges
                calc_df["Operating Charge (MYR)"] = ((calc_df["Depth Charge (per ft)"]*total_depth) + calc_df["Flat Rate"])*(1-discount)
                calc_df["Rental Charge (MYR)"] = (calc_df["Quantity of Tools"]*((calc_df["Daily Rate"]*total_days)+(calc_df["Monthly Rate"]*total_months)))*(1-discount)
    ...
    def export_to_excel(all_calc_dfs_for_excel, selected_ref_well):
        ...
        headers = ["Ref", "Specification 1", "Specification 2", "Daily Rate", "Monthly Rate", "Depth Charge (per ft)", "Flat Charge", "Operating Charge", "Rental Charge", "Total (MYR)"]
        ...
        for hole_size, used_special_cases, df_tools_section, special_cases_section in all_calc_dfs_for_excel:
            if selected_ref_well != "None":
                ws[f"A{current_row}"] = selected_ref_well
