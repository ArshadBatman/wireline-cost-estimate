    ...
    for tab, hole_size in zip(tabs, hole_sizes):
        with tab:
           ...
           # --- Package & Service ---
            ...
            # --- Calculation ---
            # --- Calculated Costs Table (Editable) ---
            # --- Filter df_tools to only include selected/expanded codes ---
            df_tools = df_service[df_service["Specification 1"].isin(expanded_codes)].copy()
            
            # --- Build Selected Data with dividers ---
            display_rows = []
            ...
            for item in df_tools["Specification 1"].unique():
                if item not in sum(special_cases.values(), []):
                    item_rows = df_tools[df_tools["Specification 1"] == item]
                    display_rows.extend([row.to_frame().T for _, row in item_rows.iterrows()])
            ...
            st.subheader(f"Selected Data - Package {selected_package}, Service {selected_service}")
            st.dataframe(selected_df.style.apply(highlight_divider, axis=1), use_container_width=True)
            
            # --- Build Calculated Costs (editable copy) ---
            ...
            # Function to recalc costs (skips divider rows)
            def recalc_costs(df):
                df = df.copy()
            ...
            # Highlight dividers in editable table
            def highlight_divider_calc(row):
                if str(row["Specification 1"]).startswith("---"):
                    return ["background-color: red; color: white"] * len(row)
                return [""] * len(row)
